...
{	 
	...
	Usart3_Init(115200);		//USART2初始化——用于与USART HMI串口屏通信
	...
	//Servo初始化————转90度
	TIM_SetCompare1(TIM1, 600);			//TIM1_CH1_PA8_垂直方向_ROLL
	//经过实验这里为17; 理论上为15;
	
	delay_ms(5000);
	TIM_SetCompare2(TIM3, 590);			//TIM3_CH2_PB5_水平方向_YAW    //SetCompare这里定义里面更改过
	//经过实验这里为16; 理论上为15;
	delay_ms(5000);
 	...
 	while(1)
	{
		...
		if(mpu_dmp_get_data(&pitch,&roll,&yaw)==0)
		{
			...
			// 	break;
			// default: //90°
			// 	angle_yaw = 590;	//、、	昨天的问题，必须和初始化一样，不然会动
			// 	break;
			// }
